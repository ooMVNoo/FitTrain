...
bool LinReg::val_in_tab(int val, vector<int>tab){
...
vector<float> LinReg::linearize(vector<vector<float>> tab){
    int col=tab[0].size();
...
vector<float> LinReg::logistic_proba(vector<vector<float>> xp){
...
vector<float> LinReg::logistic_prediction(vector<vector<float>> xp,float seuil){
